...
class Hosting_manager_model extends Crud_model {

    protected $table = null;

    function __construct() {
        // Define the table for diagrams
        $this->table = 'hosting_account';
    ...
    public function get_details($options = array()) {
        ...
        if ($project_id) {
            $where .= " AND $details_table.project_id=" . intval($project_id);
        }
    
        $client_id = $this->_get_clean_value($options, "client_id");
        if ($client_id) {
            $where .= " AND $details_table.client_id=" . intval($client_id);
        }
    
        $category = $this->_get_clean_value($options, "category");
        if ($category) {
            $where .= " AND $details_table.category=" . $this->db->escape($category);
        }
    
        $name = $this->_get_clean_value($options, "search_by");
        if ($name) {
            $name = $this->db->escapeLikeString($name);
            $where .= " AND $details_table.title LIKE '%" . $name . "%' ESCAPE '!'";
        }
